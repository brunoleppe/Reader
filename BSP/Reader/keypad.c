...
_Noreturn void keypad_task(void *params)
{
    ...
    while(true) {
        ...
        if(key != 0) {
            prev_key = key;
            input_report_key(prev_key, INPUT_EVENT_PRESSED);
            counter = 0;
            xTimerStart(clickTimer, 0);
        }
        else{
            xTimerStop(clickTimer, 0);
            input_report_key(prev_key, INPUT_EVENT_RELEASED);
            input_report_key(prev_key, INPUT_EVENT_CLICKED);
            if(counter >= 2 && counter < 4)
                input_report_key(prev_key, INPUT_EVENT_LONG_CLICKED);
            else if(counter >= 4)
                input_report_key(prev_key, INPUT_EVENT_LONG_LONG_CLICKED);
            prev_key = 0;
        }
    }
}